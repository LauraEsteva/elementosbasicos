class Table extends HTMLElement {
    ...
    //lo segundo que arrancará será el connectedCallback. Arranca cuando la etiqueta html
    // es cargada en el documento. webcomponents3 classroom, funciones

    attributeChangedCallback(){
        this.loadData();
    }

    //esta función arrancara cuando un atributo sea creado o cambie. 
    ...
    filter(search){
        ...
        rows.forEach(row => {
            ...
            row.innerText.toLowerCase().indexOf(search);
            //console.log(row.innerText.toLowerCase().indexOf(search));
            if (row.innerText.toLowerCase().indexOf(search)>-1) {
                row.style.display = '';
            }
            if (row.innerText.toLowerCase().indexOf(search)==-1) {
